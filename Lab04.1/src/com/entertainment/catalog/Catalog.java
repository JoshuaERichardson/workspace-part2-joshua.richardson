...
public class Catalog {
    ...
    public static Collection<Television> findByBrand(String brand) {
        return catalog.stream().filter(a -> a.getBrand().equals(brand)).collect(Collectors.toList());
    }
    ...
    public static Map<String, Collection<Television>> findByBrands(String... brands) {
        ...
        for(var brand : brands){
            var found = findByBrand(brand);
            if (!found.isEmpty()) tvs.put(brand,found);
        }
        return tvs;
    }
     ...
     * @return
     */
    public static List<Object> getInventory() {
        return List.of(catalog.toArray());
    }
